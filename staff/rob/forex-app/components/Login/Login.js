class Log_In extends HTMLComponent {
    ...
    async asyncLogin(u, p) {
        ...
        let c = await authenticateUser(user.username, user.password);
        if (c) {
            that.ErrorElement.classList.remove("label--error--display");
            let cc = await retrieveUser(c);
            current_user = cc.t;
            current_user.password = p;
            current_user.auth_token = c;
            current_user.forex_token = await retrieveForexToken(user.username, user.password, 'demo');
    ...
    login(u, p) {
        ...
        modelservice$.publish("loading", true);
        authenticateUser(user.username, user.password).then((c) => {
            if (c) {
                that.ErrorElement.classList.remove("label--error--display");
